...
async function handleSubscriptionCreated(supabaseClient: any, body: any) {
  try {
    ...
    // Update user's tier in users table based on subscription amount
    if (body.data.metadata.user_id) {
      ...
      if (amount >= 1999)
        ...
        tier = "pro"; // $9.99/month
      else if (amount >= 499) tier = "basic"; // $4.99/month

      console.log(
        `Updating user ${body.data.metadata.user_id} tier to ${tier} based on amount ${amount}`,
      );

      const { data: updateData, error: userUpdateError } = await supabaseClient
        .from("users")
        .update({ tier })
        .eq("id", body.data.metadata.user_id)
        .select();

      if (userUpdateError) {
        console.error("Error updating user tier:", userUpdateError);
      } else {
        console.log(`Successfully updated user tier:`, updateData);
      }
    }
  ...
  try {
    ...
    if (activeSub) {
      ...
      if (body.data.metadata.user_id) {
        ...
        console.log(
          `Activating user ${body.data.metadata.user_id} tier to ${tier} based on amount ${amount}`,
        );

        const { data: updateData, error: userUpdateError } =
        ...
        if (userUpdateError) {
          ...
          console.log(`Successfully activated user tier:`, updateData);
        }
      }
    }
...
async function handleSubscriptionCanceled(supabaseClient: any, body: any) {
  try {
    ...
    if (canceledSub) {
      ...
      // Update user's tier back to free
      if (body.data.metadata.user_id) {
        console.log(
          `Canceling subscription for user ${body.data.metadata.user_id}, reverting to free tier`,
        );
        ...
        if (userUpdateError) {
          console.error("Error updating user tier to free:", userUpdateError);
        } else {
          console.log(`Successfully reverted user to free tier:`, updateData);
        }
      }
    }
...
async function handleSubscriptionRevoked(supabaseClient: any, body: any) {
  try {
    ...
    if (revokedSub) {
      ...
      if (body.data.metadata.user_id) {
        console.log(
          `Revoking subscription for user ${body.data.metadata.user_id}, reverting to free tier`,
        );
        ...
        if (userUpdateError) {
          ...
          console.log(
            `Successfully reverted user to free tier after revocation:`,
            updateData,
          );
        }
      }
